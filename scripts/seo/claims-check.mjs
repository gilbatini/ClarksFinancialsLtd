...
const patterns = [
  ...
  { label: "unsupported current licence assertion", expression: /Clarks Financials Limited is a licensed|Licensed Tier-4 Institution|Licensed Institution ·|fully regulated by UMRA|operates under the full oversight/gi },
  { label: "unsupported turnaround statistic", expression: /\b24\s*hrs?\b|within 24 hours?|decisions? within 24/gi },
  { label: "unsupported founding statistic", expression: /\b(?:est\.?|since|incorporated in)\s*2012\b|serving since/gi },
  { label: "unsupported branch-count claim", expression: /\btwo locations\b|\bbranch at Kireka\b/gi },
  { label: "unsupported numeric product term", expression: /starting (?:from )?UGX\s*[\d,]+|(?:up to|above|min(?:imum)?\.?)\s+UGX\s*[\d,]+|\b1\s*(?:—|-|to)\s*24 months\b|\bup to 24 months\b/gi },
  { label: "unsupported eligibility rule", expression: /permanently employed|aged 18 years|two guarantors required|monthly salary of UGX/gi },
